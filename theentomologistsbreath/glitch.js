...
window.addEventListener("load", () => {
    ...
    function drawBodyFragments(){
        for(let i = bodyFragments.length-1; i>=0; i--){
            ...
            ctx.drawImage(images[currentImg], f.sx,f.sy,f.sw,f.sh,f.dx,f.dy,f.dw,f.dh);
    ...
    function randomPoints(sw, sh, n){
        const pts = [];
        for(let i=0;i<n;i++) pts.push({x:rand(-EDGE_JITTER,EDGE_JITTER), y:rand(-EDGE_JITTER,EDGE_JITTER)});
        return pts;
    }
    ...
    function initSections(){
        ...
        for(let i=0;i<SECTION_COUNT;i++){
            ...
            if(Math.random()<0.38){
                ...
                cx = rand(0 + sw/2, canvas.width - sw/2);
                cy = rand(0 + sh/2, canvas.height - sh/2);
            }
            sections.push({
                sx: cx - sw/2, sy: cy - sh/2, sw, sh, cx, cy,
            ...
            sections[i].edgePts = randomPoints(sw, sh, INTERNAL_POINTS);
            sections[i].baseEdgePts = sections[i].edgePts.map(p=>({x:p.x, y:p.y}));
        }
    }
    ...
    function drawStructuralSections(img, alpha){
        ...
        sections.forEach(sec=>{
            ...
            ctx.beginPath();
            const pts = sec.edgePts;
            pts.forEach((p,i)=>{
                const x = -sec.sw/2 + (i/(pts.length-1))*sec.sw + p.x;
            ...
            if(sec.scale<0.35 || sec.cx-halfW<0 || sec.cx+halfW>canvas.width || sec.cy-halfH<0 || sec.cy+halfH>canvas.height){
                ...
                sec.cx = canvas.width/2; sec.cy = canvas.height/2;
    ...
    function draw(){
        ...
        if(alpha>0 && images[currentImg] && images[sectionImg]){
            updateImage();
            updateSectionImage();
            for(let i=2;i<5;i++){
